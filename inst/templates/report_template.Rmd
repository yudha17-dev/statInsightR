---
title: "Regression Model Report"
output:
  html_document:
    theme: cosmo
    toc: true
    toc_float: true
    number_sections: true
params:
  model: NULL
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)
library(knitr)
library(kableExtra)
library(statInsightR)

m <- params$model

# ---- Safe Table Helper ----
safe_kable <- function(df) {
  if (is.null(df) || nrow(df) == 0 || ncol(df) == 0) {
    cat("No valid results available for this test.\n")
    return(invisible(NULL))
  }
  kable(df, format = "html") %>%
    kable_styling(full_width = FALSE)
}
```

# Overview
This document provides a complete statistical report for the fitted regression model.

**Model Formula:**

```{r}
m$call
```

# Clean Summary

```{r}
summary_clean(m)
```

# Statistical Diagnostics

## Multicollinearity (VIF)

```{r}
library(car)
num_terms <- length(attr(terms(m), "term.labels"))

if (num_terms < 2) {
  cat("VIF not available: model has fewer than 2 predictors.\n")
} else {
  vif_res <- try(vif(m), silent = TRUE)

  if (inherits(vif_res, "try-error")) {
    cat("Could not compute VIF for this model.\n")
  } else {
    df_vif <- data.frame(
      Variable = names(vif_res),
      VIF = round(vif_res, 3)
    )
    safe_kable(df_vif)
  }
}
```

## Normality of Residuals (Shapiro-Wilk Test)

```{r}
sh <- try(shapiro.test(residuals(m)), silent = TRUE)

if (inherits(sh, "try-error") ||
    is.null(sh$statistic) ||
    is.null(sh$p.value) ||
    length(sh$statistic) == 0 ||
    length(sh$p.value) == 0 ||
    any(is.na(sh$statistic)) ||
    any(is.na(sh$p.value))) {
  
  cat("Shapiro-Wilk test could not be computed for this model.\n")
  
} else {
  df_sh <- data.frame(
    Statistic = as.numeric(sh$statistic),
    p_value = as.numeric(sh$p.value)
  )
  safe_kable(df_sh)
}

```

## Heteroskedasticity (Breusch–Pagan Test)

```{r}
library(lmtest)
bp <- try(bptest(m), silent = TRUE)

if (inherits(bp, "try-error") ||
    is.null(bp$statistic) ||
    is.null(bp$p.value) ||
    length(bp$statistic) == 0 ||
    length(bp$p.value) == 0 ||
    any(is.na(bp$statistic)) ||
    any(is.na(bp$p.value))) {
  
  cat("Breusch–Pagan test could not be computed for this model.\n")
  
} else {
  df_bp <- data.frame(
    Statistic = as.numeric(bp$statistic),
    p_value = as.numeric(bp$p.value)
  )
  safe_kable(df_bp)
}


```

## Influential Points (Cook’s Distance)

```{r}
ci <- cooks.distance(m)
n <- length(ci)

df_ci <- data.frame(
  Index = seq_along(ci),
  Cook = ci,
  Influential = ifelse(ci > (4/n), "Yes", "No")
)

df_ci <- df_ci[order(-df_ci$Cook), ]

cat("### Top 10 Most Influential Points\n\n")
safe_kable(head(df_ci, 10))
```

<details>
<summary><strong>Click to expand full Cook’s Distance table (all points)</strong></summary>

<br>

```{r}
safe_kable(df_ci)
```

</details>

# Diagnostic Plots

## Residuals vs Fitted

```{r}
plot(m, which = 1)
```

## Normal Q–Q Plot

```{r}
plot(m, which = 2)
```

## Cook’s Distance Plot

```{r}
plot(m, which = 4)
```

# Conclusion

```{r}
cat(statInsightR::auto_conclusion(m))
```

This report was generated using **statInsightR**.
